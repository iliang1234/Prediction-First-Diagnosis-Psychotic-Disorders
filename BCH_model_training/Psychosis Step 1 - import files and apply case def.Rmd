---
title: "Psychosis - Import Data and Apply Case Definition"
output:
...
APPLY_CASE_DEFINITION <- FALSE

```

```{r rld_dta, eval=RELOAD_DATA}
print("RELOAD_DATA")
```

```{r apply_case, eval=APPLY_CASE_DEFINITION}
print("APPLY_CASE_DEFINITION")
```

## Define Global Functions
```{r define_functions, include = FALSE, echo=FALSE, warning=FALSE}
...
```{r load_files, include = FALSE, echo = FALSE, eval = RELOAD_DATA}

# demographics
demographics = readAllDatasetSegments('./Demographics2')
demographics$BIRTH_DATE <- as.POSIXct(strptime(demographics$BIRTH_DATE, format = '%Y-%m-%d %H:%M:%OS'))
demographics$SEX_CD <- factor(demographics$SEX_CD)
demographics$MARITAL_STATUS_CD <- factor(demographics$MARITAL_STATUS_CD)
...
# diagnosis
diagnosis = readAllDatasetSegments('./Diagnosis2')
...
```{r load_previously_saved_datasets, eval = (!RELOAD_DATA)}
...
```{r inclusion_criteria, eval = RELOAD_DATA}
...
included_ids = unique(all_subjects$PATIENT_ID[all_subjects$BIRTH_DATE >= MIN_BIRTH_DATE & all_subjects$BIRTH_DATE < MAX_BIRTH_DATE])
saveRDS(included_ids, 'included_ids.RDs') 

```

```{r load_included_ids, eval = (!RELOAD_DATA)}
included_ids = readRDS('included_ids.RDs') 
```

```{r subsample_datasets, eval = RELOAD_DATA}
...
```{r load_subsample_datasets, eval = (!RELOAD_DATA)}
diagnosis.included = readRDS('diagnosis.included.RDs')
demographics.included = readRDS('demographics.included.RDs')
